...
import com.huanghua.mychat.ui.MainActivity;
import com.huanghua.mychat.ui.NewFriendActivity;
...
public class MyMessageReceiver extends BroadcastReceiver {
                    ...
                    if (TextUtils.isEmpty(tag)) {// 不携带tag标签
                        ...
                        if (toId.equals(currentUser.getObjectId())) {// 只有当toId和当前登陆用户相符合才继续向下传递和弹通知，防止别人恶意刷消息
                            ...
                            if (ehList.size() > 0) {// 有监听的时候，传递下去
                                ...
                                if (isAllow && currentUser != null
                                        && currentUser.getObjectId().equals(toId)) {// 当前登陆用户存在并且也等于接收方id
                                    mNewNum++;
                        ...
                        if (tag.equals(BmobConfig.TAG_ADD_CONTACT)) {
                            ...
                            if (isAllow && currentUser != null
                                ...
                                BmobNotifyManager.getInstance(context).showNotify(
                                        R.drawable.ic_launcher, tickerText, username,
                                        tickerText.toString(), MainActivity.class);
                            }
                            ...
                            // 更改某条消息的状态
                            BmobChatManager.getInstance(context).updateMsgStatus(
                                    BmobConfig.STATUS_SEND_RECEIVERED, conversionId, msgTime);
                            if (toId.equals(currentUser.getObjectId())) {
    ...
    public void showNotification(Context context, BmobMsg msg) {
        ...
        BmobNotifyManager.getInstance(context).showNotifyWithExtras(icon, tickerText.toString(),
